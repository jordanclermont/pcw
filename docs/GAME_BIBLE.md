...
move) added as the human's main crowd-work tool (§5.11); the game opens on the
booking screen again with a one-key skip (§8). Supersedes the v1.6 note that
the match boots straight into the ring.*
...
- **The rating is the crowd (Build 0.13, re-balanced 0.14)**: the PCW Observer rating is itemized, and the results card prints every line. **Crowd** = a blend of average heat (60%) and average front-row lean (40%), divided by 20. Bonuses: **trust held** (ended at 80+) +¼, **clean finish** +¼, **respected** (both workers' average Respect 65+) +¼. Penalties: each **botch** −0.3; each **off-sheet** move costs in proportion to how serious the liberty was (its trust cost ÷ 60 — a stray strike about −0.1, an off-script slam about −0.25, kicking out of the real finish −0.5). A by-the-book match that bores the front row can't be a classic, and one mistake costs a fraction of a star, not a whole one. All knobs in `PCW.RATING` (config.js).
  Measured in the harness (two CPUs, 0.14): The Classic — played by the sheet 3¾★, **played to the room (posing after each move) 4½★**, with one botch 3★. The War — by the sheet 2¾★ (big moves before the crowd is ready burn its belief), played to the room 4½★. Pre-0.13, every flawless run scored 5★. *Why 0.14 re-balanced:* 0.13 lowered the crowd side but left the old penalties (−0.6 per botch, −0.4 per shoot) in place, so any real human match with a mistake or two capped around 2½★ (Jordan's playtest).
...
**Posing — the well-timed taunt (Build 0.14).** A taunt right after *your own* move (within about 2½ seconds, once per move) is selling the moment: the pose over the fallen man. The crowd pays it about 2½× a taunt out of nowhere and the front row leans in; a heel posing over his work also banks extra resentment. It is still subject to "they've seen it" fatigue, so posing after every single move wears thin. This is the main tool, today, for a human to out-perform the CPU (which never taunts) — measured +¾ to +1¾ stars over a by-the-sheet run.
